...
def create_visualization_report(
    ...
    if not HAS_MATPLOTLIB:
        ...
        if not placeholder_path.exists():
            with open(placeholder_path, "w") as f:
                f.write(
                    "Matplotlib not available for visualization generation.\nInstall matplotlib to enable visualization features."
                )
    ...
    if style_config is None:
        style_config = {"figure_size": (20, 10), "dpi": 300, "palette": "husl", "font_size": 10, "title_font_size": 14}
    ...
    fig.suptitle(
        f"Psychological Data Analysis Report (N={len(data)})", fontsize=style_config.get("title_font_size", 14), y=0.98
    )
    ...
    for i, (x_col, y_col, title) in enumerate(scatter_pairs):
        try:
            ...
            if HAS_SCIPY:
                # Guard against NaN/inf values that crash scipy's linregress
                if any(not np.isfinite(v) for v in x_vals + y_vals):
                    continue
...
def create_time_series_visualization(
    model_data: pd.DataFrame, output_dir: str = "docs/figures", filename: str = "time_series.pdf"
) -> str:
    """
    ...
    if not HAS_MATPLOTLIB:
        ...
        if not placeholder_path.exists():
            with open(placeholder_path, "w") as f:
                f.write(
                    "Matplotlib not available for time series visualization generation.\nInstall matplotlib to enable visualization features."
                )
        return str(placeholder_path)
    ...
    # Default style configuration
    style_config = {"figure_size": (16, 10), "dpi": 300, "font_size": 10, "title_font_size": 12}
    ...
    fig.suptitle(
        "Time Series Analysis: Mental Health Metrics", fontsize=style_config.get("title_font_size", 14), y=0.95
    )
    ...
    for idx, (col, label) in enumerate(variables):
        ...
        for window, color, alpha in zip(moving_windows, ma_colors, alphas[1:]):
            if len(plot_data) >= window:
                ...
                ax.plot(plot_data.index, ma, color=color, linewidth=2, label=f"{window}-step MA", alpha=alpha)
