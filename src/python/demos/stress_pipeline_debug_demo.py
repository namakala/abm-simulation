...
class StressPipelineDebugger:
        ...
        for step in range(self.max_steps):
            ...
            if step % 10 == 0:
                ...
                if self.model.agents:
                    ...
                    print(
                        f"  Debug: Agent 0 - Stress: {agent.current_stress:.3f}, PSS-10: {agent.pss10}, Events: {len(getattr(agent, 'daily_stress_events', []))}"
                    )
    ...
    def _initialize_previous_values(self):
        ...
        for i, agent in enumerate(self.model.agents):
            ...
            self.previous_values[agent_id] = {"current_stress": agent.current_stress, "pss10_score": agent.pss10}
        ...
        for agent in self.model.agents:
            ...
            if daily_events:
                ...
                coping_prob = compute_coping_probability(
                    latest_event["challenge"], latest_event["hindrance"], neighbor_affects, StressProcessingConfig()
                )
    ...
    def _analyze_step_correlations(self, step: int):
        ...
        event_indices = [
            i
            for i in range(len(self.debug_data["challenge"]))
        ...
        try:
            ...
            if step % 10 == 0:
                print(
                    f"  Debug: Step {step + 1} correlations (events={len(event_indices)}) - Resilience-PSS10: {resilience_pss10_corr:.4f}, Affect-PSS10: {affect_pss10_corr:.4f}"
                )
    ...
    def _analyze_resilience_coping_relationship(self) -> Dict[str, Any]:
        ...
        if len(unique_resilience) <= 3:
            ...
            quartile_stats = (
                df.groupby("resilience_group")
                .agg({"coping_success": ["mean", "count", "std"], "resilience": ["mean", "min", "max"]})
                .round(4)
            )
        else:
            ...
            try:
                df["resilience_quartile"] = pd.qcut(
                    df["resilience"], 4, labels=["Q1", "Q2", "Q3", "Q4"], duplicates="drop"
                )

                quartile_stats = (
                    df.groupby("resilience_quartile")
                    .agg({"coping_success": ["mean", "count", "std"], "resilience": ["mean", "min", "max"]})
                    .round(4)
                )
            except ValueError:
    ...
    def _analyze_coping_probability_mechanism(self) -> Dict[str, Any]:
        ...
        for prob_bin in prob_bins:
            ...
            calibration_data.append({"predicted_prob": prob_bin, "actual_success_rate": actual_rate, "count": total})
        ...
        for data in calibration_data:
            print(
                f"  Predicted {data['predicted_prob']:.1f}: Actual {data['actual_success_rate']:.3f} (n={data['count']})"
            )
    ...
    def _analyze_correlation_evolution(self) -> Dict[str, Any]:
        ...
        for corr_name, expected_dir in expected_directions.items():
            ...
            status = (
                "✓"
                if (
                    (expected_dir == "negative" and actual_dir == "negative")
                    or (expected_dir == "positive" and actual_dir == "positive")
                )
                else "✗"
            )
    ...
    def _validate_stress_processing_pipeline(self) -> Dict[str, Any]:
        ...
        if len(controllability_vals) > 1 and len(pss10_scores) > 1:
            ...
            controllability_changes = [
                controllability_vals[i] - controllability_vals[i - 1] for i in range(1, len(controllability_vals))
            ]
            pss10_changes = [pss10_scores[i] - pss10_scores[i - 1] for i in range(1, len(pss10_scores))]
