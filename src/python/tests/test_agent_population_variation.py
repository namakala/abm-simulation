...
class PopulationAnalyzer:
    ...
    def create_visualization_report(self, agents: List[Person], filename: str = None) -> str:
        """
        ...
        if not HAS_MATPLOTLIB:
            ...
            placeholder_path = self.output_dir / "visualization_not_available.txt"
        ...
        axes[0, 0].axvline(np.mean(resilience_vals), color='red', linestyle='--', label=f'Mean: {np.mean(resilience_vals):.3f}')
        axes[0, 0].legend()
        ...
        axes[0, 1].axvline(np.mean(affect_vals), color='red', linestyle='--', label=f'Mean: {np.mean(affect_vals):.3f}')
        axes[0, 1].legend()
        ...
        axes[0, 2].axvline(np.mean(resources_vals), color='red', linestyle='--', label=f'Mean: {np.mean(resources_vals):.3f}')
        axes[0, 2].legend()
        ...
        axes[2, 2].axvline(np.mean(pss10_vals), color='red', linestyle='--', label=f'Mean: {np.mean(pss10_vals):.1f}')
        axes[2, 2].legend()
    ...
    def generate_summary_report(self, agents: List[Person], config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        viz_path = self.create_visualization_report(agents)
...
class TestAgentPopulationVariation:
    ...
    def test_visualization_generation(self, analyzer, sample_config):
        ...
        viz_path = analyzer.create_visualization_report(agents)
