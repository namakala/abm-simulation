...
| `NETWORK_WATTS_K` | $k$ | 4 | Mean degree in Watts-Strogatz network topology |
| `NETWORK_WATTS_P` | $p_{\text{rewire}}$ | 0.1 | Rewiring probability in Watts-Strogatz network |
...
| `NETWORK_HOMOPHILY_STRENGTH` | - | 0.7 | Strength of homophily in network connections |
| `AGENT_INITIAL_RESILIENCE` | $R_{\text{baseline}}$ | 0.5 | Initial resilience level for agents |
| `AGENT_INITIAL_AFFECT` | $A_{\text{base}}$ | 0.0 | Initial affect level for agents |
...
| `AGENT_SUBEVENTS_PER_DAY` | - | 3 | Number of subevents per day for each agent |
...
| `STRESS_BETA_ALPHA` | $\alpha$ | 2.0 | Alpha parameter for Beta distribution of stress events |
| `STRESS_BETA_BETA` | $\beta$ | 2.0 | Beta parameter for Beta distribution of stress events |
...
| `THRESHOLD_BASE_THRESHOLD` | $\eta$ | 0.5 | Base threshold for stress responses |
...
| `THRESHOLD_AFFECT_THRESHOLD` | $\lambda_{\text{affect}}$ | 0.3 | Affect threshold for homeostasis |
...
| `PSS10_LOAD_CONTROLLABILITY` | $\lambda_{ij}$ | [0.2, 0.8, 0.1, 0.7, 0.6, 0.1, 0.8, 0.6, 0.7, 0.1] | Factor loadings for controllability dimension |
| `PSS10_LOAD_OVERLOAD` | $\lambda_{ij}$ | [0.7, 0.3, 0.8, 0.2, 0.4, 0.9, 0.2, 0.3, 0.4, 0.9] | Factor loadings for overload dimension |
...
| `PSS10_THRESHOLD` | - | 27 | Threshold for PSS-10 stress classification |
...
| `INTERACTION_RESILIENCE_INFLUENCE` | $\beta_r$ | 0.05 | Resilience influence in interactions |
| `INTERACTION_MAX_NEIGHBORS` | - | 10 | Maximum neighbors for social influence |
...
| `RESILIENCE_OVERLOAD_THRESHOLD` | - | 3 | Threshold for overload effects |
| `RESILIENCE_BOOST_RATE` | $\theta_{\text{boost}}$ | 0.1 | General boost rate for resilience |
| `N_INFLUENCING_NEIGHBORS` | - | 5 | Number of influencing neighbors |
| `N_INFLUENCING_HINDRANCE` | - | 3 | Number of hindrance-influencing neighbors |
...
| `RESOURCE_ALLOCATION_COST` | - | 0.15 | Cost of resource allocation |
...
| `UTILITY_SOFTMAX_TEMPERATURE` | $\beta$ | 1.0 | Temperature parameter for softmax decisions |
